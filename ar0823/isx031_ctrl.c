...
struct tegracam_ctrl_ops isx031_ctrl_ops = {
...
static const struct v4l2_ctrl_ops isx031_v4l2_ctrl_ops = {
	.s_ctrl = isx031_s_ctrl,
};

struct v4l2_ctrl_config isx031_custom_controls[] = {
	{
		.ops = &isx031_v4l2_ctrl_ops,
...
unsigned int isx031_num_custom_controls = ARRAY_SIZE(isx031_custom_controls);
 ...
 * isx031_custom_controls, and then merges it with Tegracam's internal
 ...
 */
int isx031_ctrls_init(struct v4l2_subdev *sd) {
	...
	v4l2_ctrl_handler_init(priv->custom_ctrl_handler, isx031_num_custom_controls);

	// Add standard controls
	ctrl = v4l2_ctrl_new_std(priv->custom_ctrl_handler, &isx031_v4l2_ctrl_ops, V4L2_CID_HFLIP, 0, 1, 1, 0);
	if (ctrl == NULL) {
		dev_err(priv->dev, "Error initializing standard control");
		return -EINVAL;
	}

	ctrl = v4l2_ctrl_new_std(priv->custom_ctrl_handler, &isx031_v4l2_ctrl_ops, V4L2_CID_VFLIP, 0, 1, 1, 0);
	if (ctrl == NULL) {
		dev_err(priv->dev, "Error initializing standard control");
		return -EINVAL;
	}

	// Add each custom control in isx031_custom_controls
	for (i = 0; i < isx031_num_custom_controls; i++) {
		dev_dbg(dev, "Registering custom control %d with v4l2", i);
		ctrl = v4l2_ctrl_new_custom(priv->custom_ctrl_handler, &isx031_custom_controls[i], NULL);
...
static int isx031_set_vflip(struct isx031 *priv, s64 val)
{
	int err = 0;
...
static int isx031_set_hflip(struct isx031 *priv, s64 val)
{
	int err = 0;
...
static int isx031_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	struct isx031 *priv =
		container_of(ctrl->handler, struct isx031,
			     _custom_ctrl_handler);
	int err = 0;

	switch (ctrl->id) {
		case V4L2_CID_HFLIP:
			err = isx031_set_hflip(priv, ctrl->val);
			break;
		case V4L2_CID_VFLIP:
			err = isx031_set_vflip(priv, ctrl->val);
			break;
